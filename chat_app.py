...
class ModelManager:
    ...
    def generate_stream(self, messages: list, max_tokens: int = 512, temperature: float = 0.7):
...
def chat_response(message: str, history: list, model_name: str, max_tokens: int, temperature: float):
    ...
    try:
        for partial_response in model_manager.generate_stream(messages, max_tokens, temperature):
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Accordion("Chat History", open=False):
            with gr.Row():
                ...
                save_status = gr.Textbox(label="Save Status", interactive=False, scale=2)
            
            with gr.Row():
                history_dropdown = gr.Dropdown(
                    choices=get_saved_chats(),
        ...
        def bot_response(history, model_name, max_tok, temp):
            ...
            # Generate response
            for response in chat_response(user_msg, history[:-2], model_name, max_tok, temp):
        ...
        msg.submit(
            ...
            bot_response,
            [chatbot, model_dropdown, max_tokens, temperature],
            chatbot
        ).then(
            on_model_loaded,
            [model_dropdown],
        ...
        save_btn.click(
            lambda h: save_chat_history(h),
            [chatbot],
            [save_status]
        )
        
        load_btn.click(
            lambda f: load_chat_history(f),
            [history_dropdown],
            [chatbot, save_status]
        )
        
        refresh_btn.click(
            lambda: gr.update(choices=get_saved_chats()),
...
def main():
    ...
    if not models:
        print("WARNING: No models found! Please ensure models are in the 'models' directory.")
